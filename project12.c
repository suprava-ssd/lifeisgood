...
void read_block(int block_num, void *buffer) {
    ...
    if (lseek(fs_fd, offset, SEEK_SET) == -1) {
        fprintf(stderr, "ERROR: Failed to seek to block %d: %s\n", block_num, strerror(errno));
        exit(1);
    }
    ssize_t bytes_read = read(fs_fd, buffer, BLOCK_SIZE);
    if (bytes_read != BLOCK_SIZE) {
        fprintf(stderr, "ERROR: Failed to read block %d: %s\n", block_num, bytes_read < 0 ? strerror(errno) : "Incomplete read");
        exit(1);
    }
}
...
void write_block(int block_num, void *buffer) {
    ...
    if (bytes_written != BLOCK_SIZE) {
        fprintf(stderr, "ERROR: Failed to write block %d: %s\n", block_num, bytes_written < 0 ? strerror(errno) : "Incomplete write");
        exit(1);
    }
}
...
void process_single_indirect(uint32_t indirect_block, uint8_t *calculated_bitmap) {
    uint32_t pointers[BLOCK_SIZE / sizeof(uint32_t)];
    read_block(indirect_block, pointers);
...
void process_double_indirect(uint32_t double_indirect_block, uint8_t *calculated_bitmap) {
    uint32_t pointers[BLOCK_SIZE / sizeof(uint32_t)];
    read_block(double_indirect_block, pointers);

    for (int i = 0; i < BLOCK_SIZE / sizeof(uint32_t); i++) {
        uint32_t single_indirect_block = pointers[i];
        if (single_indirect_block != 0 && single_indirect_block >= DATA_BLOCK_START && single_indirect_block < TOTAL_BLOCKS) {
            set_bit(calculated_bitmap, single_indirect_block);
            process_single_indirect(single_indirect_block, calculated_bitmap);
        }
    }
}

void process_triple_indirect(uint32_t triple_indirect_block, uint8_t *calculated_bitmap) {
    uint32_t pointers[BLOCK_SIZE / sizeof(uint32_t)];
    read_block(triple_indirect_block, pointers);

    for (int i = 0; i < BLOCK_SIZE / sizeof(uint32_t); i++) {
        uint32_t double_indirect_block = pointers[i];
        if (double_indirect_block != 0 && double_indirect_block >= DATA_BLOCK_START && double_indirect_block < TOTAL_BLOCKS) {
            set_bit(calculated_bitmap, double_indirect_block);
            process_double_indirect(double_indirect_block, calculated_bitmap);
        }
    }
}
        ...
        if (inode->links_count > 0 && inode->dtime == 0) {
            ...
            if (inode->single_indirect != 0) {
                if (inode->single_indirect < DATA_BLOCK_START || inode->single_indirect >= TOTAL_BLOCKS) {
                    ...
                    process_single_indirect(inode->single_indirect, calculated_bitmap);
                }
            }
            ...
            if (inode->double_indirect != 0) {
                if (inode->double_indirect < DATA_BLOCK_START || inode->double_indirect >= TOTAL_BLOCKS) {
                    ...
                    process_double_indirect(inode->double_indirect, calculated_bitmap);
                }
            }
            ...
            if (inode->triple_indirect != 0) {
                if (inode->triple_indirect < DATA_BLOCK_START || inode->triple_indirect >= TOTAL_BLOCKS) {
                    ...
                    process_triple_indirect(inode->triple_indirect, calculated_bitmap);
                }
            }
        }
    }
...
void check_and_fix_fs_image(const char *filename) {
    ...
    if (fs_fd == -1) {
        fprintf(stderr, "ERROR: Failed to open file system image: %s\n", strerror(errno));
        exit(1);
    }
    ...
    if (fsync(fs_fd) == -1) {
        fprintf(stderr, "ERROR: Failed to sync changes to disk: %s\n", strerror(errno));
        close(fs_fd);
        exit(1);
    }
    ...
    if (close(fs_fd) == -1) {
        fprintf(stderr, "ERROR: Failed to close file system image: %s\n", strerror(errno));
        exit(1);
    }
}
